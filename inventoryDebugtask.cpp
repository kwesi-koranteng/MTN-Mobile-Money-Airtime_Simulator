...
class Inventory {
private:
    std::vector<Item*> items;  // Changed from array to vector
    float total_money;
...
public:
    ...
    void add_item() {
        ...
        items.push_back(new Item(name, quantity, price));  // Add to vector
    }
    ...
    void remove_item(int item_index) {
        ...
        if (input_quantity <= quantity) {
            ...
            // Completely remove item if quantity is zero
            if (item->get_quantity() == 0) {
                delete item;  // Free up memory
                items.erase(items.begin() + item_index);  // Remove from vector
            }
        } else {
